/* Netflix Official Dual Subtitles v1.3 for Shadowrocket.
 * Current selected official track on top, previously selected official track below.
 * Keeps each Netflix Range response as the fixed cue skeleton and only rewrites cue text.
 * Netflix cue line breaks are flattened and an empty lower track uses an invisible placeholder.
 */
...
function overlappingText(cue, bottom) {
  const seen = new Set();
  const text = [];
  for (const candidate of bottom) {
    if (candidate.s >= cue.e) break;
    if (candidate.e <= cue.s || seen.has(candidate.t)) continue;
    seen.add(candidate.t);
...
function merge(body, currentBody, otherBody) {
  ...
  return `${part.map((cue, index) => {
    const matched = matches[index];
    const lower = matched ? overlappingText(matched, bottom) : "";
